...
function processEventData(cfg, messageBody) {
  ...
  function checkRequiredFields(cfg, messageBody) {
    ...
    if (!messageBody.start) {
      return defer.reject(new Error("Start Time missing! This field is required!"));
    }
    if (!messageBody.end) {
      return defer.reject(new Error("End Time missing! This field is required!"));
    }
    if (!cfg.timeZone) {
      return defer.reject(new Error("Time Zone missing! This field is required!"));
    }
    if (!cfg.calendarId) {
      return defer.reject(new Error("Calendar ID missing! This field is required!"));
    }
    if ((cfg.bodyContentType) && (!messageBody.body)) {
      return defer.reject(new Error("Body Type provided, but Body Content is missing!"));
    }

    return buildPostBody(cfg, messageBody);
  }
  ...
  function buildPostBody() {
    ...
    function formatDate(date, timeZone, format) {
      ...
      return defer.reject(new Error("Invalid date " + date));
    }
