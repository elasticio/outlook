...
function checkRequiredFields(logger, cfg, messageBody) {
...
function formatDate(logger, date, timeZone, format) {
  // Valid date string
  if (moment(date).isValid()) {
    // Valid ISO_8601 string
    if ((moment(date, moment.ISO_8601).isValid())) {
      logger.info(`Date ${date} has a known ISO-8601 format.`);
...
function buildPostBody(logger, cfg, messageBody) {
  checkRequiredFields(logger, cfg, messageBody);
  ...
  if ((cfg.isAllDay) && (cfg.isAllDay === 'true')) {
    ...
    result.start.dateTime = formatDate(logger, result.start.dateTime.trim(), cfg.timeZone, FORMAT);
    result.end.dateTime = formatDate(logger, result.end.dateTime.trim(), cfg.timeZone, FORMAT);
...
async function processEventData(logger, cfg, messageBody) {
  logger.info('Processing event data ...');
  const defer = Q.defer();
  try {
    const result = buildPostBody(logger, cfg, messageBody);
